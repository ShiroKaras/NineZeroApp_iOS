...
- (instancetype)initWithFrame:(CGRect)frame Type:(SKMascotType)mascotType isHad:(BOOL)isHad {
	...
	if (self) {
		...
		_mascotSkillIntroArray = @[@"",
					   ...
					   @"想要像零仔Pride·W一样时刻闪耀在聚光灯下其实很简单，只要请求他给你的你的头像加一个blingbling的魔法边框就好啦（效果持续3天）",
...
- (void)hintS1ButtonClick:(UIButton *)sender {
	[HTProgressHUD show];
	[[[SKServiceManager sharedInstance] propService] purchasePropWithPurchaseType:@"1"
									     propType:@"1"
									     callback:^(BOOL success, NSString *responseString, NSInteger coolTime) {
										 [HTProgressHUD dismiss];
...
- (void)hintS2ButtonClick:(UIButton *)sender {
	[HTProgressHUD show];
	[[[SKServiceManager sharedInstance] propService] purchasePropWithPurchaseType:@"2"
									     propType:@"1"
									     callback:^(BOOL success, NSString *responseString, NSInteger coolTime) {
										 [HTProgressHUD dismiss];
										 [self showPromptWithText:responseString];
...
- (void)exchangeButtonClick:(UIButton *)sender {
	[[[SKServiceManager sharedInstance] mascotService] useMascotSkillWithMascotID:_mascotIdArray[_type]
