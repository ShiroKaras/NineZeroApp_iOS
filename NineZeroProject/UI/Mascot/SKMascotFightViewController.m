...
- (void)viewDidLoad {
	...
	if (!NO_NETWORK) {
		[[[SKServiceManager sharedInstance] mascotService] getRandomStringWithMascotID:[NSString stringWithFormat:@"%ld", (long)_mascot.pet_id]
...
- (void)onClickMonster:(UITapGestureRecognizer *)sender {
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.033 * images.count * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
	    [[[SKServiceManager sharedInstance] mascotService] mascotBattleWithMascotID:[NSString stringWithFormat:@"%ld", (long)_mascot.pet_id]
