...
- (void)showAnswerPropAlertView {
	...
	if (_season == 1)
		[cancelButton setTitleColor:COMMON_GREEN_COLOR forState:UIControlStateHighlighted];
	else if (_season == 2)
		[cancelButton setTitleColor:COMMON_PINK_COLOR forState:UIControlStateHighlighted];
...
- (void)usePropButtonOnClick:(UIButton *)sender {
	...
	[[[SKServiceManager sharedInstance] answerService] answerExpiredTextQuestionWithQuestionID:self.currentQuestion.qid
										  answerPropsCount:[NSString stringWithFormat:@"%ld", (long)self.currentQuestion.num]
											  callback:^(BOOL success, SKResponsePackage *response) {
											      if (response.result == 0) {
												      //回答正确
...
- (void)createGiftViewWithButton:(UIButton *)button reward:(NSDictionary *)reward ticket:(NSDictionary *)ticket {
	...
	[rewardBaseInfoView mas_makeConstraints:^(MASConstraintMaker *make) {
	    ...
	    make.centerX.equalTo(rewardBackView);
	    if (isTicket)
		    make.top.equalTo(@((rewardBackView.height - card.height - 294 - 20) / 2));
	    else
		    make.top.equalTo(@((rewardBackView.height - 294) / 2));
	}];
