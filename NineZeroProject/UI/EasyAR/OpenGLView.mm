    ...
    namespace samples {
        ...
        void HelloARVideo::render()
        {
            ...
            if(status == AugmentedTarget::kTargetStatusTracked){
                ...
                if (!tracked_target) {
                    if (video == NULL) {
                        if(frame.targets()[0].target().name() == std::string("qianmen") && texid[0]) {
                            video = new ARVideo;
                            video->openTransparentVideoFile("qianmen.mp4", texid[0]);
                            video_renderer = renderer[0];
                        }
                        else if(frame.targets()[0].target().name() == std::string("nanluo") && texid[1]) {
                            video = new ARVideo;
                            video->openTransparentVideoFile("nanluo.mp4", texid[1]);
                            video_renderer = renderer[1];
                        }
                        else if(frame.targets()[0].target().name() == std::string("shuilifang") && texid[2]) {
                            video = new ARVideo;
                            video->openTransparentVideoFile("shuilifang.mp4", texid[2]);
                            video_renderer = renderer[2];
                        }
                        else if(frame.targets()[0].target().name() == std::string("niaochao") && texid[3]) {
                            video = new ARVideo;
                            video->openTransparentVideoFile("niaochao.mp4", texid[3]);
...
- (void)start{
    ar.initCamera();
    ar.loadFromImage("qianmen.jpg", 0);
    ar.loadFromImage("nanluo.jpg", 1);
    ar.loadFromImage("shuilifang.jpg", 2);
    ar.loadFromImage("niaochao.jpg", 3);
    ar.start();
