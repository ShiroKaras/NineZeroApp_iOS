...
- (void)onCaptureMascotSuccessful {
    ...
    if (_swipeType == 0) {
        ...
        SKARRewardController *rewardController = [[SKARRewardController alloc] initWithRewardID:[[[[SKServiceManager sharedInstance] questionService] questionList] lastObject].reward_id questionID:[[[[SKServiceManager sharedInstance] questionService] questionList] lastObject].qid];
        if (IOS_VERSION >= 8.0) {
